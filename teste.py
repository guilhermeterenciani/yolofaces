...
if __name__ == '__main__':
    ...
    #cap = cv2.VideoCapture("rtsp://admin:cameraip123@10.87.10.17:554/live/ch0")
    #cap.set(3,1280)
    #cap.set(4,720)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, x in enumerate(im0s):
            ...
            try:
                ...
                print("Analise:IdentificarFace %.4f ClassificarFace %.4f TimePerFrame %.4f"%(idf,classface,total))
            except:
                print(traceback.format_exc())
                print(path[i])
                
            cv2.imshow(path[i], frame)
        if args.save:
